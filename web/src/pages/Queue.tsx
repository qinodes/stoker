...
export function Queue() {
  ...
  return <>
    <PageHeading eyebrow={t("queue.breadcrumb")} title={t("queue.title")} description={t("queue.description")} actions={<><button className="button secondary" data-action="refresh" onClick={() => void actions.loadData()}>{t("queue.refresh")} <span aria-hidden="true">↻</span></button><button className="button primary" data-queue-lock="true" disabled={locked} onClick={() => void actions.queueLock(true)}>{t("queue.lock")}</button><button className="button secondary" data-queue-lock="false" disabled={!locked} onClick={() => void actions.queueLock(false)}>{t("queue.unlock")}</button></>} />

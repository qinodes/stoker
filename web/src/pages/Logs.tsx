...
export function Logs() {
  ...
  const message = state.logs.error || data?.message || (captureError ? `${stream} capture degraded: ${captureError}` : "") || (!state.jobs.length ? "No jobs are available yet. Add a job with the CLI first." : (!available && selectedJob ? `No ${stream} log is available for this job yet.` : ""));
  return <>

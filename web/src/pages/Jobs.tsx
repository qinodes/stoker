...
export function JobForm() {
  ...
  return <div className="dialog-card job-card"><div className="dialog-language"><LanguagePicker /></div><div className="dialog-kicker">{t("jobs.breadcrumb")}</div><div className="job-dialog-heading"><div><h2 id="job-dialog-title">{t("jobs.newTitle")}</h2><p>{t("jobs.newDescription")}</p></div><button className="dialog-close" data-action="close-job-form" type="button" aria-label={t("jobs.closeNew")} onClick={actions.closeJobForm}>×</button></div>
    <form id="new-job-form" noValidate onSubmit={(event) => { event.preventDefault(); void actions.saveJob(); }}>
      ...
      <div className="job-field"><label htmlFor="job-cwd">{t("jobs.directory")}</label><div className="path-input-row"><input className="text-input mono-input" id="job-cwd" value={draft.cwd} placeholder={t("jobs.directoryPlaceholder")} autoComplete="off" autoCapitalize="off" spellCheck={false} required onChange={(event) => actions.updateDraft({ cwd: event.target.value })} /><button className="button secondary" data-action="browse-directory" type="button" onClick={() => void actions.loadDirectory(draft.cwd || state.filesystem.roots?.default_path || "")}>{t("jobs.browse")}</button></div><small className="form-help">{t("jobs.directoryHelp")}</small></div>
      {(browser || state.filesystem.loading) && <div className="fs-browser"><div className="fs-path-row"><input className="text-input" id="fs-path" value={state.filesystem.inputPath || browser?.path || ""} onChange={(event) => actions.setFilesystemInputPath(event.target.value)} /><button className="button secondary" data-action="open-directory" type="button" onClick={() => void actions.loadDirectory(state.filesystem.inputPath)}>{t("common.open")}</button></div>{state.filesystem.error && <div className="form-feedback invalid">{state.filesystem.error}</div>}<div className="fs-directory-list">{state.filesystem.loading ? <EmptyState icon="⌁" title={t("jobs.loadingDirectories")} /> : browser?.directories.length ? browser.directories.map((entry) => <button className="fs-directory-row" type="button" data-directory={entry.path} key={entry.path} onClick={() => void actions.loadDirectory(entry.path)}><span className="fs-folder-glyph">◆</span><span>{entry.name}</span><span className="fs-row-arrow">›</span></button>) : <EmptyState icon="○" title={t("jobs.noDirectories")} />}</div>{browser && <button className="button small primary" data-action="choose-directory" type="button" onClick={actions.chooseDirectory}>{t("jobs.useDirectory")}</button>}</div>}

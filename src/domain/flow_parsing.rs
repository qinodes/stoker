...
pub fn parse_once(value: &str) -> Result<DateTime<Utc>, String> {
    ...
    if !has_offset || value.contains("-00:00") || value.contains("-00:00") {
        return Err("--at must be RFC 3339 with seconds and a UTC offset".to_owned());
    }
    ...
    if !seconds_part {
        return Err("--at must include seconds".to_owned());
    }
    ...
    if is_leap_second {
        return Err("--at does not accept leap-second timestamps".to_owned());
    }
    DateTime::parse_from_rfc3339(value)
        .map(|value| value.with_timezone(&Utc))
        .map_err(|error| format!("invalid --at RFC 3339 timestamp: {error}"))
}
